...
static hwc_module_t *get_hwc(void) {
    ...
    if (!orghwc) {
        ...
        handle = dlopen("/system/lib/hw/hwcomposer.vendor.tegra.so", RTLD_LAZY);
        if (!handle) {
...
static void copy_display_contents_1_to_layer_list(hwc_layer_list_t *dst,
                                                  hwc_display_contents_1_t *src) {
    dst->flags = src->flags;
    unsigned int s, d;
...
static void copy_layer_list_to_display_contents_1(hwc_display_contents_1_t *dst,
                                                  hwc_layer_list_t *src) {
    dst->flags = src->flags;
    unsigned int s, d;
...
static int tegra2_set(struct hwc_composer_device_1 *dev, size_t numDisplays,
    ...
    if (!contents->dpy || !contents->sur) return 0;

    // If blanking, make gralloc handle everything to aovid crashes
    if (pdev->fbblanked) return -ENODEV;
...
static int tegra2_prepare(hwc_composer_device_1_t *dev, size_t numDisplays,
                          hwc_display_contents_1_t **displays) {
    if (!numDisplays || !displays) return 0;
...
static void *tegra2_hwc_emulated_vsync_thread(void *data) {
    struct tegra2_hwc_composer_device_1_t *pdev = (struct tegra2_hwc_composer_device_1_t *)data;
    ...
    while (1) {
        ...
        while (pdev->fbblanked && pdev->vsync_running) {
            // When framebuffer is blanked, there must be no interrupts, so we
            // can't
            // wait on it
            pthread_cond_wait(&pdev->vsync_cond, &pdev->vsync_mutex);
        }
        if (!pdev->vsync_running) break;
...
static int dc0_get_vblank_syncpt(void) {
    ...
    if (dc0_fd < 0) dc0_fd = open("/dev/tegra_dc_0", O_RDWR);  // Older interface
    if (dc0_fd < 0) {
        ALOGE(
            "Failed to open NVidia DC0 - Assuming default VBLANK0 syncpoint "
            "id");
    ...
    if (ioctl(dc0_fd, TEGRA_DC_EXT_GET_VBLANK_SYNCPT, &syncpt) < 0) {
        ALOGE(
            "Failed to get VBLANK0 syncpoint id - Assuming default VBLANK0 "
            "syncpoint id");
        close(dc0_fd);
        return NVSYNCPT_VBLANK0;
    }
...
static int nvhost_syncpt_read(int ctrl_fd, int id, unsigned int *syncpt) {
    ...
    ra.id = id;
    if (ioctl(ctrl_fd, NVHOST_IOCTL_CTRL_SYNCPT_READ, &ra) < 0) return -1;
    *syncpt = ra.value;
...
static int tegra2_eventControl(struct hwc_composer_device_1 *dev, int dpy, int event, int enabled) {
    struct tegra2_hwc_composer_device_1_t *pdev = (struct tegra2_hwc_composer_device_1_t *)dev;
...
static int tegra2_query(struct hwc_composer_device_1 *dev, int what, int *value) {
    ...
    int ret = (!pdev->org->query) ? -EINVAL : pdev->org->query(pdev->org, what, value);
...
static void tegra2_registerProcs(struct hwc_composer_device_1 *dev, hwc_procs_t const *procs) {
    struct tegra2_hwc_composer_device_1_t *pdev = (struct tegra2_hwc_composer_device_1_t *)dev;
    pdev->procs = procs;
    if (pdev->org->registerProcs) pdev->org->registerProcs(pdev->org, procs);
}
...
static int tegra2_close(hw_device_t *device) {
    struct tegra2_hwc_composer_device_1_t *pdev = (struct tegra2_hwc_composer_device_1_t *)device;
    ...
    if (pdev->prepare_xlatebuf) free(pdev->prepare_xlatebuf);
    if (pdev->set_xlatebuf) free(pdev->set_xlatebuf);
...
static int tegra2_open(const struct hw_module_t *module, const char *name,
    ...
    if (dev->fb_fd >= 0 && ioctl(dev->fb_fd, FBIOGET_VSCREENINFO, &info) != -1) {
        unsigned long long refreshRate =
            1000000000000LLU / (uint64_t(info.upper_margin + info.lower_margin + info.yres) *
        ...
        ALOGV(
            ...
            "height       = %d mm (%f dpi)\n"
            "refresh rate = %d Hz\n",
            dev->xres, dev->yres, info.width, dev->xdpi / 1000.0, info.height, dev->ydpi / 1000.0,
            refreshRate);
    }
    ...
    if (dev->org->query && dev->org->query(dev->org, HWC_VSYNC_PERIOD, &value) == 0 && value != 0) {
        ...
        if (dev->fb_fd >= 0) {
            value = (uint64_t(info.upper_margin + info.lower_margin + info.yres) *
                     (info.left_margin + info.right_margin + info.xres) * info.pixclock) /
                    1000ULL;
