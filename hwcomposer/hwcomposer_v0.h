...
typedef struct hwc_methods {
    ...
    int (*eventControl)(struct hwc_composer_device* dev, int event, int enabled);
} hwc_methods_t;
...
typedef struct hwc_composer_device {
     ...
     *
     */
    int (*set)(struct hwc_composer_device* dev, hwc_display_t dpy, hwc_surface_t sur,
     ...
     */
    void (*registerProcs)(struct hwc_composer_device* dev, hwc_procs_t const* procs);
...
static inline int hwc_open(const struct hw_module_t* module, hwc_composer_device_t** device) {
    return module->methods->open(module, HWC_HARDWARE_COMPOSER, (struct hw_device_t**)device);
}
